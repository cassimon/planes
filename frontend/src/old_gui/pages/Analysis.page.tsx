...
const LazyPlot = React.lazy(async () => {
  ...
  const factory =
    (createPlotlyComponent as any).default ?? createPlotlyComponent
...
function generateScatterPlotData() {
  ...
  return {
    ...
    layout: {
      ...
      xaxis: {
        title: { text: "Annealing Temperature (°C)" },
        gridcolor: "#e9ecef",
      },
      yaxis: { title: { text: "PCE (%)" }, gridcolor: "#e9ecef" },
      paper_bgcolor: "rgba(0,0,0,0)",
      ...
      <Group justify="space-between" p="sm">
        <Group gap="sm">
          ...
          <Badge size="xs" variant="light">
            {experiment.substrates.length} substrates × {columns.length}{" "}
            parameters
          </Badge>
        </Group>
        <Group gap="sm">
        ...
        <ScrollArea h={200} p="sm" pt={0}>
          {columns.length === 0 ? (
            <Text size="sm" c="dimmed" ta="center" py="lg">
              ...
              {filterMode === "varied"
                ? "No varied parameters in this experiment."
                : "No layers with parameters."}
            </Text>
          ) : (
